...
bool ManualMap(HANDLE hProc, const BYTE* pDllData, size_t dllSize) {
	printf("Injecting...\n");
	const BYTE* pSrcData = pDllData;
	size_t srcSize = dllSize;
	if (srcSize == 0)
		return false;
	BYTE* pTargetBase = nullptr;

	if (reinterpret_cast<const IMAGE_DOS_HEADER*>(pSrcData)->e_magic != 0x5A4D) {
		printf("[-] e_magic != 0x5A4D\n");
		return false;
	}

	const IMAGE_NT_HEADERS* pOldNtHeader = reinterpret_cast<const IMAGE_NT_HEADERS*>(pSrcData + reinterpret_cast<const IMAGE_DOS_HEADER*>(pSrcData)->e_lfanew);
	const IMAGE_OPTIONAL_HEADER* pOldOptHeader = &pOldNtHeader->OptionalHeader;
	const IMAGE_FILE_HEADER* pOldFileHeader = &pOldNtHeader->FileHeader;
